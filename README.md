...
5. “打开已有文章”会显示内容管线，可按构思、草稿、待审核、可发布和已发布筛选；状态修改只进入编辑区，仍需按原流程审核并保存。
6. 用“发布就绪检查”只读核对元数据、失效或未公开的 WikiLink、缺失附件、平台转换提醒、反向链接和相关文章。
7. 明确确认后保存 Markdown，再一次生成五个平台物料：CSDN / 掘金使用标准 Markdown，知乎 / 微信公众号使用富文本兼容稿，小红书同时生成纯文本长文和可下载的 3:4 PNG 图卡。
8. 预览并确认复盘条目后，才会回写 Obsidian 元知识与写作事件日志。
...
- 发布检查默认把没有明确公开标志的内部链接视为未公开；检查只读，不会自动公开笔记、附件或修改原文。
